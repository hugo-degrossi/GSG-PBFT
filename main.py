from GSGPBFT import run_GSGPBFT
from GSGPBFT import *
from client import *
...
nodes[0]=[("faulty_primary",0),("non_responding_node",0),("slow_nodes",10),("faulty_node",0),("honest_node",50), ("faulty_replies_node",0),("moving_node",15)] # Nodes starting from the beginning
...
run_GSGPBFT(nodes=nodes,proportion=p,checkpoint_frequency0=checkpoint_frequency,clients_ports0=clients_ports,timer_limit_before_view_change0=timer_limit_before_view_change)

time.sleep(1) # Waiting for the network to start...

# Run clients:
requests_number = 101  # The user chooses the number of requests he wants to execute simultaneously (They are all sent to the PBFT network at the same time) - Here each request will be sent by a different client
clients_list = []
...
for i in range (requests_number):
    ...
    time.sleep(7) #Exécution plus rapide lorsqu'on attend un moment avant de lancer la requête suivante